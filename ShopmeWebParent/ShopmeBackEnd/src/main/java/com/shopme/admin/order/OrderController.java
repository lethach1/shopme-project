...
public class OrderController {
	...
	private void updateOrderTracks(Order order, HttpServletRequest request) {
		...
		for (int i = 0; i < trackIds.length; i++) {
			OrderTrack trackRecord = null;

			Integer trackId = Integer.parseInt(trackIds[i]);
			if (trackId > 0) {
				// Tìm bản ghi cũ trong orderTracks nếu nó đã tồn tại
				trackRecord = orderTracks.stream().filter(track -> track.getId().equals(trackId)).findFirst()
						.orElse(new OrderTrack());
				trackRecord.setId(trackId);
			} else {
				trackRecord = new OrderTrack();
			}
