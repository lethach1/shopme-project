...
public class WebSecurityConfig {
	 ...
	 * UserDetailsService với quá trình xác thực. Toàn bộ quy trình xác thực
	 * (username, password) sẽ dc xử lý bên trong phương thức aythenticate
	 */
	@Bean
	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(authorize -> authorize.requestMatchers("/states/list_by_country/**")
				.hasAnyAuthority("Admin", "Salesperson")
				.requestMatchers("/users/**", "/settings/**", "/countries/**", "/states/**").hasAuthority("Admin")
				.requestMatchers("/categories/**", "/brands/**", "articles/**", "/sections/**")
				...
				.requestMatchers("/reviews/**", "/questions/**").hasAnyAuthority("Admin", "Assistant").anyRequest()
				.authenticated())
